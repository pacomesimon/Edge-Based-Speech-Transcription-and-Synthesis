...
class Encoder(nn.Module):
    ...
    def forward(self, x, mask=None, is_from_edge = False, get_embeddings = False, x_embed = None):
        if not is_from_edge:
          seq_len = x.shape[1]
...
class Decoder(nn.Module):
    ...
    def forward(self, mel, encoder_output, src_mask, casual_mask, is_from_edge = False, get_embeddings = False, x_embed = None):
        if not is_from_edge:
          seq_len = mel.shape[1]
          prenet_out = self.prenet(mel)
          x = prenet_out + self.positional_embed[:, :seq_len, :]
        else:
          x = x_embed
        if get_embeddings:
          return x
...
class TransformerTTS(nn.Module):
    ...
    def forward(self, text_idx, mel, mel_mask, is_from_edge = False, get_embeddings = False, x_embed = None, y_embed = None):
        if not is_from_edge:
          input_pad_mask = self.input_mask(text_idx)
          target_pad_mask, causal_mask = self.target_mask(mel, mel_mask)
          if get_embeddings:
            encoder_embed_out = self.encoder(text_idx, input_pad_mask, get_embeddings = True)
            encoder_out = self.encoder(text_idx, input_pad_mask)
            decoder_embed_out = self.decoder(mel, encoder_out, target_pad_mask, causal_mask, get_embeddings = True)
            return encoder_embed_out, decoder_embed_out
          else:
            encoder_out = self.encoder(text_idx, input_pad_mask)
            ...
            encoder_out = self.encoder(None, None, is_from_edge = True, x_embed = x_embed)
            mel_postout, mel_linear, stop_linear = self.decoder(mel, encoder_out, None, None, # None(s) 
                                                                is_from_edge = True, x_embed = y_embed)
            return mel_postout, mel_linear, stop_linear
    
    def get_encoder_output(self, text_idx):
        input_pad_mask = self.input_mask(text_idx)
        encoder_out = self.encoder(text_idx, input_pad_mask)
        return encoder_out

    def get_decoder_output(self, encoder_out, mel, mel_mask):
        target_pad_mask, causal_mask = self.target_mask(mel, mel_mask)
        mel_postout, mel_linear, stop_linear = self.decoder(mel, encoder_out, target_pad_mask, causal_mask)
        return mel_postout, mel_linear, stop_linear
